...
import { apiClient, type MenuItem, type OrderItemRequest, type OrderRequest } from '@/services/apiClient'
...
const submitOrder = async () => {
  if (!customerName.value || cart.value.length === 0) {
    message.value = 'Please enter name and add items'
    return
  }
  ...
  try {
    const res = await apiClient.createOrder(req)
    message.value = `Order created! Ticket #${res.ticketNumber}`
    cart.value = []
    customerName.value = ''
  } catch (e: any) {
...
<template>
  <div class="split-layout">
    ...
    <div class="order-form clipboard">
      <div class="clipboard-clip"></div>
      <h2 class="form-title">Regular Order</h2>
      
      <div class="form-container">
        <div class="input-group">
          <label>Customer Name:</label>
          <input v-model="customerName" type="text" placeholder="John Doe" />
        </div>
        <div class="input-group">
          <label>Type:</label>
        ...
        <button class="action-btn" @click="submitOrder" :disabled="cart.length === 0">Submit Order</button>
...
.clipboard {
  ...
  position: relative;
  box-shadow: 8px 8px 0px rgba(15, 23, 42, 0.1); /* Subtle solid shadow to lift it off the gray workbench */
}
.clipboard-clip {
  position: absolute;
  top: -10px;
